...
var postHandler = function(req, res, next) {
    ...
    if (req.method === 'POST') {
        ...
        //convert forms if request is convert forms
        if(req.url.toString().endsWith('xlsx-convert-all')){
            const testFolder = './app/config';
            let arrFiles = findAllxlsxFiles(testFolder, 'xlsx');
            ...
            res.write('converted successfully');
            res.end();
        }
...
var convertTest = (grunt) =>{
    grunt.log.writeln('test from grunt files');
};
function findAllxlsxFiles(startDir,ext,files,result){
    var pth = require('path');
    var fs = require('fs');
    files = files || fs.readdirSync(startDir) 
    result = result || [] 

    files.forEach( file => {
            var newbase = pth.join(startDir,file)
            if ( fs.statSync(newbase).isDirectory() ) {
                result = findAllxlsxFiles(newbase,ext,fs.readdirSync(newbase),result)
            }
            else{
                if ( file.substr(-1*(ext.length+1)) == '.' + ext ){
                    newbase = newbase.replace('app\\', '');
                    newbase = newbase.replaceAll('\\', '/');
                    result.push(newbase)
                } 
            }
        }
